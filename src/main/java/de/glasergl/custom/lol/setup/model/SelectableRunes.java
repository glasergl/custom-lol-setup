...
public class SelectableRunes {
    ...
    public Set<Rune> getSelected() {
	final Set<Rune> selectedRunes = new HashSet<>();
	for (final List<SelectableRune> runeRow : runes) {
	    for (final SelectableRune rune : runeRow) {
		if (rune.isSelected()) {
		    selectedRunes.add(rune.getRune());
		}
	    }
	}
